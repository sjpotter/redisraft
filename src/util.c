...
unsigned int keyHashSlot(RedisModuleString *key) {
    ...
    if (s == key_len) return crc16_ccitt(key_str,key_len) & 0x3FFF;
    ...
    if (e == key_len || e == s+1) return crc16_ccitt(key_str,key_len) & 0x3FFF;

    /* If we are here there is both a { and a } on its right. Hash
     * what is in the middle between { and }. */
    return crc16_ccitt(key_str+s+1,e-s-1) & 0x3FFF;
}
